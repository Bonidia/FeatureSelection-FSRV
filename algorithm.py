...
def set_options():
	if len(sys.argv) == 4:
		...
		file_name = input('Dataset file name: ')

	print(f'\nAlgorithm option: {alg_option_dict[alg_option]}')
	print(f'Classifier option: {clf_option_dict[clf_option]}')
	print(f'File name: {file_name}')

	dataset_name = file_name.split('/')[-1].split('(')[0].split('.')[0]
	dataset = pd.read_csv(file_name)

	return alg_option, clf_option, dataset, dataset_name
...
alg_option, clf_option, dataset, dataset_name = set_options()
voted_options = [3]
...
for s in range(sampling + 1):
	...
	X, y, column_names = split_X_y(dataset)
